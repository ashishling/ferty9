...
const TIMEOUT_MS = 900000; // 15 minutes timeout for background functions
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Configure axios with longer timeout for background function
    ...
    // For background functions, we can return the result directly
    // since we have 15 minutes to work with
    return NextResponse.json({ transcription });

  } catch (error: any) {
    console.error('Error transcribing audio (background):', error.response ? error.response.data : error.message);
